...
(function(){
  ...
  function makeCat(el, fur, stripe, opts){
    ...
    function perimeterPoint(t, w, h){
      ...
      // top gets extra clearance so the cat walks below the sticky navbar
      var inset = 14;
      var topInset = 58;
      var W = w - inset*2, H = h - inset - topInset;
      var per = 2*(W+H);
      var d = ((t % 1) + 1) % 1 * per;

      if (d < W){ return { x: inset + d, y: topInset, edge:'top' }; }
      d -= W;
      if (d < H){ return { x: w - inset, y: topInset + d, edge:'right' }; }
      d -= H;
